setwd("/Users/jannawilloughby/GDrive/Willoughby lab/furbearer abundance /proposal/meta analysis/")
setwd("~/R/")
...
#study site size
lm.out=lm(data$ncamgen.resid~log(data$study.area.size.km2)) 
summary(lm.out)
...
#This was put on hold to look at a better way to compare this 
##trapping vs noninvasive comparison
lm.out=glm(data$noninv.resid~data$habitat.biogeo)
summary(lm.out)
boxplot(data$noninv.resid~data$habitat.biogeo) #differences here, maybe we change this to a half violin plot? boxplots are ugly...
table(data$habitat.biogeo)

lm.out=glm(data$noninv.resid~data$habitat.biogeo)   
summary(lm.out)
boxplot(data$noninv.resid~data$habitat.biogeo)
table(data$habitat.biogeo)

##compare within noninvasive methods
lm.out=glm(data$ncamgen.resid~data$habita.types.fixed)
summary(lm.out)
