...
#ran the regression taking out the 0 for intercept and the log to find the intercept which was the new average of 7 point something
lm.out=lm((data$noninv)~(data$livetrap.num.indv))
summary(lm.out)
...
exp(mean(data$noninv.resid, na.rm=T))
###Answer: yes but there is a lot of variance around the 1:1 line, mean of 1 individual difference in estimates.
#tendency to find more individuals with non invasive compared to live trapping method.
...
#same thing happened here as step #1 where we took off the 0 since it would give a mean of 1 and took off the log
lm.out=lm((data$genetic.num.indv)~(data$camera.num.indv))
summary(lm.out)
...
exp(mean(data$ncamgen.resid, na.rm=T))
...
##trapping vs noninvasive comparison
lm.out=glm(data$noninv.resid~data$habitat.biogeo)
summary(lm.out)
...
lm.out=glm(data$noninv.resid~data$habitat.types)   #i'm not convinced we have the best reclass atm. for example, what does mountain mean? taiga maybe?
summary(lm.out)
boxplot(data$noninv.resid~data$habitat.types)
table(data$habitat.types)

##compare within noninvasive methods
lm.out=glm(data$ncamgen.resid~data$habitat.biogeo)
summary(lm.out)
table(data$habita.types.fixed[data$comparison=="camgen"])
boxplot(data$ncamgen.resid~data$habita.types.fixed)

lm.out=glm(data$ncamgen.resid~data$habitat.types)
summary(lm.out)
table(data$habitat.types[data$comparison=="camgen"])
boxplot(data$ncamgen.resid~data$habitat.types)
